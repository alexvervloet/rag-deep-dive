...
few broad ones. More focused isn't automatically better, since a too-small chunk may
...
It computes cosine similarity against *every* stored vector, then sorts, so O(n) per
...
is huge, so one match is decisive. Term frequency barely matters (the code occurs
...
but different MRR, and the one that ranks the right chunk #1 instead of #4 gives the
...
the corpus means the cache is stale, so `--rebuild` re-embeds. The cache also auto-
